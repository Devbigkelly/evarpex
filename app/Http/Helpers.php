...
if (!function_exists('renderStarRating')) {
    function renderStarRating($rating, $maxStars = 5)
    {
        ...
        $html = '<div class="d-flex gap-1 fs-sm">';
        ...
        for ($i = 1; $i <= $maxStars; $i++) {
            if ($rating >= $i) {
                ...
                // Half star
                $html .= '<i class="ci-star-half text-warning"></i>';
            } else {
                // Empty star
                $html .= '<i class="ci-star text-muted"></i>';
            }
        }
...
if (!function_exists('getLastViewedProducts')) {
    ...
    {
        // If user is not logged in, return empty collection
        if (!auth()->check()) {
            return collect();
        }
        ...
        return LastViewedProduct::where('user_id', auth()->id())
            ...
            ->whereIn('product_id', function ($query) use ($verified_sellers) {
                $query->select('id')
                    ...
                    ->when(!addon_is_activated('auction'), function ($q) {
                        $q->where('auction_product', 0);
                    })
                    ->when(get_setting('vendor_system_activation') == 0, function ($q) {
                        $q->where('added_by', 'admin');
                    })
                    ->when(get_setting('vendor_system_activation') == 1, function ($q) use ($verified_sellers) {
                        $q->where(function ($p1) use ($verified_sellers) {
                            $p1->where('added_by', 'admin')
