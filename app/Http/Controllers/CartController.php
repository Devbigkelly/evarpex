   ...
   public function addToCart(Request $request)
{
    ...
    if ($authUser) {
        ...
        $temp_user_id = $request->session()->get('temp_user_id') 
            ?? tap(bin2hex(random_bytes(10)), fn ($id) => $request->session()->put('temp_user_id', $id));
    ...
    // 🔥 NORMALIZED QUANTITY (HERE IS THE FIX)
    ...
    if ($quantity < ($product->min_qty ?? 1)) {
        return [
            'status' => 0,
            'cart_count' => count($carts),
            'modal_view' => view('frontend.partials.minQtyNotSatisfied', [
